...
--logs-dir ./logs/RGA-SC/cuhk03labeled_b64f2048
--evaluate
--resume ./logs/RGA-SC/cuhk03labeled_b64f2048/checkpoint_300.pth.tar
"""
...
def main(args):
	...
	## Evaluator
	evaluator = ImgEvaluator(model, file_path=args.logs_dir) #evaluator只是使用一个网络结构和路劲
	# test/evaluate the model
	if args.evaluate:
		feats_list = ['feat_', 'feat']	#feat_(16,2048),bn得到feat（16，2048）
		evaluator.eval_worerank(query_loader, gallery_loader, dataset.query, dataset.gallery, #query查询，gallery画廊，走廊
			metric=['cosine'], 		#使用余弦相似度
			types_list=feats_list)	#特征
		return
...
if __name__ == '__main__':
	...
	parser.add_argument('--margin', type=float, default=0.3,
						help="margin of the triplet loss, default: 0.3") #margin是一个大于0的常数。最终的优化目标是拉近a和p的距离，拉远a和n的距离
	# optimizer
