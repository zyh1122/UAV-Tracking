...
class AMF_STrack(STrack):
    ...
    def AMF_update(self, new_track, frame_id):
        ...
        # init kalman tlwh2xyah                 tlwh2xyar         tlwh2xywh
...
class UAVMOT(BaseTracker):
    ...
    def update(self, det_results, ori_img):
        ...
        if self.NMS:
            ...
            nms_indices = nms(torch.from_numpy(AMF_STrack.xywh2tlbr(det_results[:, :4])), torch.from_numpy(det_results[:, 4]), 
        ...
        if det_high.shape[0] > 0:
            if self.use_apperance_model:
                features = self.get_feature(AMF_STrack.xywh2tlbr(det_high[:, :4]), ori_img)
                # detections: List[Strack]
                D_high = [AMF_STrack(cls, AMF_STrack.xywh2tlwh(xywh), score, kalman_format=self.opts.kalman_format, feature=feature)
                                for (cls, xywh, score, feature) in zip(det_high[:, -1], det_high[:, :4], det_high[:, 4], features)]
            else:
                D_high = [AMF_STrack(cls, AMF_STrack.xywh2tlwh(xywh), score, kalman_format=self.opts.kalman_format)
                            for (cls, xywh, score) in zip(det_high[:, -1], det_high[:, :4], det_high[:, 4])]
        else:
            D_high = []

        if det_low.shape[0] > 0:
            D_low = [AMF_STrack(cls, AMF_STrack.xywh2tlwh(xywh), score, kalman_format=self.opts.kalman_format)
