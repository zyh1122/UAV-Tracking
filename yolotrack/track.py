...
def set_basic_params(cfgs):
    global CATEGORY_DICT, DATASET_ROOT, CERTAIN_SEQS, IGNORE_SEQS, YAML_DICT #category_doct, dataset_root, certain_seqs, ignore_seqs, yaml_dict
...
timer = Timer()
seq_fps = []  # list to store time used for every seq  创建空列表用于存储时间
def main(opts, cfgs):
    ...
    }  # dict for trackers, key: str, value: class(BaseTracker) 建立一个追踪器的字典

    # NOTE: ATTENTION: make kalman and tracker compatible  一些追踪器有自己专门的滤波器：sort（NaiveKalmanFilter），botsort（BoTSORTKalmanFilter），srtongsort（NSAKalmanFilter），
    ...
    ckpt = torch.load(opts.model_path, map_location=device) #map_location参数是用于重定向，比如此前模型的参数是在cpu中的，我们希望将其加载到cuda:0中。或者我们有多张卡，那么我们就可以将卡1中训练好的模型加载到卡2中，这在数据并行的分布式深度学习中可能会用到。
    model = ckpt['ema' if ckpt.get('ema') else 'model'].float().fuse().eval()  # for yolo v7 ： fuse()是用来进行conv和bn层合并，为了提速模型推理速度。 eval()是模型进行预测推理时关闭BN（预测数据均值方差计算）和Dropout以免影响预测结果。
    ...
    if opts.data_format == 'yolo':
       ...
       # DATA_ROOT = 'F:\\python_model\\Yolov7-tracker-master\\datasets\\VisDrone2019-MOT\\VisDrone2019-MOT-val\\sequences'
        DATA_ROOT = 'F:\\python_model\\Yolov7-tracker-master\\datasets\\VisDrone2019-MOT\\VisDrone2019-MOT-test-dev\\sequences'
        seqs = os.listdir(DATA_ROOT)
    else:
        raise NotImplementedError #未执行的错误
    seqs = sorted(seqs) #sorted 可以对所有可迭代的对象进行排序操作。
...
#开始对视频进行追踪
    for seq in seqs:
        ...
        for i, (img, img0) in enumerate(data_loader):
            pbar.update()
            timer.tic()  # start timing this img              #及时模块
        ...
        timer.clear()  # clear for next seq  时间清零
        pbar.close()
    ...
    if opts.track_eval:
        default_eval_config = trackeval.Evaluator.get_default_eval_config()  #trackeval.eval.Evaluator. get_default_eval_config 有这个函数的定义，里面有一些默认参数的定义
        default_dataset_config = trackeval.datasets.MotChallenge2DBox.get_default_dataset_config() #trackeval.datasets.MotChallenge2DBox.get_default_dataset_config 有这个函数，里面是关于数据集的一些默认参数
        ...
        default_metrics_config = {'METRICS': ['HOTA', 'CLEAR', 'Identity'], 'THRESHOLD': 0.5} #参数指标 字典
        config = {**default_eval_config, **default_dataset_config, **default_metrics_config}  # Merge default configs 默认的配置信息
        ...
        dataset_list = [trackeval.datasets.MotChallenge2DBox(dataset_config)] if opts.dataset in ['mot', 'uavdt'] else [trackeval.datasets.VisDrone2DBox(dataset_config)] #MotChallenge2DBox 或者 VisDrone2DBox
        metrics_list = []
        for metric in [trackeval.metrics.HOTA, trackeval.metrics.CLEAR, trackeval.metrics.Identity, trackeval.metrics.VACE]:  #用到的参数有HOTA, CLEAR, Identity， VACE？？？？
...
#        evaluate(sorted(os.listdir(f'./tracker/tracker/results/{folder_name}')),
               sorted([seq + '.txt' for seq in seqs]), data_type='visdrone', result_folder=folder_name)
#folder.name 文件夹名字  seq_name 序列名字，
...
if __name__ == '__main__':
    ...
    parser.add_argument('--tracker', type=str, default='deepsort', help='sort, deepsort, bytetrack, deepmot,botsort,uavmot,strongsort,etc')
    ...
    """For tracker"""
    # model path
    #parser.add_argument('--reid_model_path', type=str, default='F:\\python_model\\Yolov7-tracker-master\\weights\\ckpt.t7', help='path for reid model path')
    #parser.add_argument('--reid_model_path', type=str, default='F:\python_model\Yolov7-tracker-master\deep_sort\deep\checkpoint\\11.11-dataset_zyh+rga+res+4wh\ckpt.t7', help='path for reid model path')
    #parser.add_argument('--reid_model_path', type=str,default='F:\\python_model\\Yolov7-tracker-master\\weights\\checkpoint_300.pth.tar',help='path for reid model path') #F:\\python_model\\Yolov7-tracker-master\\weights\\checkpoint_300.pth.tar
    parser.add_argument('--dhn_path', type=str, default='F:\\python_model\\Yolov7-tracker-master\\weights\\DHN.pth', help='path of DHN path for DeepMOT')
    ...
    with open(f'F:\\python_model\\Yolov7-tracker-master\\tracker\\config_files\\{opts.dataset}.yaml', 'r') as f:
        cfgs = yaml.load(f, Loader=yaml.FullLoader)#读取 yaml文件， Fullloader--加载完整的YAML语言。避免任意代码执行。这是当前（PyYAML5.1）默认加载器调用，BaseLoader - -仅加载最基本的YAML；SafeLoader - -安全地加载YAML语言的子集。建议用于加载不受信任的输入。
    main(opts, cfgs)
