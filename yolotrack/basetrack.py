...
class STrack(BaseTrack):
    def __init__(self, cls, tlwh, score, kalman_format='default', 
        ...
        # info of this track
        self.cls = cls
        self._tlwh = np.asarray(tlwh, dtype=np.float32)  # init tlwh #将结构数据转化为ndarray。
        self.score = score
    ...
    def tlwh(self):
        """
        update current bbox with kalman mean 不同卡尔曼滤波器更新位置的方法不一样
        """
    ...
    def multi_predict(stracks, kalman):
        """
        predict many tracks  多轨迹预测
        stracks: List[class(STrack)]
        """
        # TODO
        if len(stracks) > 0:
            multi_mean = np.asarray([st.mean.copy() for st in stracks])
            multi_covariance = np.asarray([st.cov for st in stracks]) #covariance 协方差矩阵
...
class BaseTracker(object):
    def __init__(self, opts, frame_rate=30, *args, **kwargs) -> None:
        ...
        self.frame_id = 0
        self.det_thresh = opts.conf_thresh #检测的阈值，
        self.buffer_size = int(frame_rate / 30.0 * opts.track_buffer) #buffer 缓存，
        self.max_time_lost = self.buffer_size

        self.NMS = True  # whether use NMS

        self.kalman = KALMAN_DICT[self.opts.kalman_format]()    #选择卡尔曼滤波的方式

        if isinstance(opts.img_size, int): #判断isinstance(opts.img_size, int),如果opts.img_size是int，则返回true
            self.model_img_size = [opts.img_size, opts.img_size]
        elif isinstance(opts.img_size, (list, tuple)):
            self.model_img_size = opts.img_size

        self.debug_mode = False #debug模式
    def update(self, det_results, ori_img):
        """
        this func is called by every time step

        det_results: numpy.ndarray or torch.Tensor, shape(N, 6), 6 includes bbox（4位）, conf_score, cls
        ...
        det_results[:, 0], det_results[:, 2] =  det_results[:, 0]*ratio[1], det_results[:, 2]*ratio[1] #乘比例 多维数据的处理 0和2
        det_results[:, 1], det_results[:, 3] =  det_results[:, 1]*ratio[0], det_results[:, 3]*ratio[0] #             1和3
        ...
        if det_results.shape[0] > 0:
            if self.NMS:
                ...
                nms_indices = nms(torch.from_numpy(bbox_temp), torch.from_numpy(det_results[:, 4]), #NMS是直接引用的函数
                                self.opts.nms_thresh)   #track.py讲NMS设置未0，7
                det_results = det_results[nms_indices.numpy()]

            # detections: List[Strack] 列表的形式
        ...
        for track in self.tracked_stracks:
            if not track.is_activated:
                unconfirmed.append(track)   #未追踪到的track
            else:
                tracked_stracks.append(track)   #如果track被激活，追踪到的track
